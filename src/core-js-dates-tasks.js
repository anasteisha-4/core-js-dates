...
function isDateInPeriod(date, period) {
  ...
  return start <= checkedDate && checkedDate <= end;
}
...
function formatDate(date) {
  ...
  hours -= hours > 12 ? 12 : 0;
  return `${dateObj.getUTCMonth() + 1}/${dateObj.getUTCDate()}/${dateObj.getFullYear()}, ${hours}:${dateObj.getMinutes().toString().padStart(2, '0')}:${dateObj.getSeconds().toString().padStart(2, '0')} ${timePeriod}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYear = (firstThursday.getDay() + 6) % 7;
  firstThursday.setDate(firstThursday.getDate() - firstDayOfYear);
  return Math.floor((target - firstThursday) / 1000 / 3600 / 24 / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  do {
    dateObj.setDate(dateObj.getDate() + 1);
  } while (dateObj.getDay() !== 5 || dateObj.getDate() !== 13);
  return dateObj;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let state = 'work';
  let daysRemained = countWorkDays;
  while (today <= dayEnd) {
    if (state === 'work') {
      workingDays.push(
      ...
      today.setDate(today.getDate() + countOffDays);
      daysRemained = countWorkDays;
      state = 'work';
    }
  }
  return workingDays;
